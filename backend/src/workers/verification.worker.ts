...
export const initializeVerificationWorker = async () => {
            ...
            try {
                ...
                for (const doc of sgpaDocs) {
                    ...
                    try {
                        ...
                        if (!response.ok)
                            throw new Error(
                                `Download failed for sem ${doc.semester}`
                            );
                        ...
                        console.log(
                            `[DEBUG] Raw Text for Sem ${doc.semester}:`,
                            text
                        );

                        // Extraction using Zod transformer
                        const zodResult =
                            verificationRawTextSchema.safeParse(text);

                        if (!zodResult.success) {
                            isOverallCorrect = false;
                            mismatchDetails.push(
                                `Sem ${doc.semester}: Extraction failed.`
                            );
                            continue;
                        }
                        ...
                        if (!validateNameTokens(profile.fullName, name)) {
                            isOverallCorrect = false;
                            mismatchDetails.push(
                                `Sem ${doc.semester}: Identity mismatch (extracted name: ${name}).`
                            );
                            continue;
                        }
                        ...
                        if (!detectedAstuRollNo) {
                            ...
                            isOverallCorrect = false;
                            mismatchDetails.push(
                                `Sem ${doc.semester}: Roll No inconsistency (${rollNo} vs ${detectedAstuRollNo}).`
                            );
                        }
                        ...
                        const backlogRegex =
                            /([A-Z]{2,}\d{3,}[A-Z0-9-]*)\s+(.*?)\s+F\s+(?:\d+)\s+Not Completed/gi;
                        let match;
                        while ((match = backlogRegex.exec(text)) !== null) {
                            detectedBacklog = true;
                            if (match[2])
                                detectedBacklogSubjects.add(match[2].trim());
                        }
                        ...
                        isOverallCorrect = false;
                        mismatchDetails.push(
                            `Sem ${doc.semester}: OCR/Structural error.`
                        );
                    }
                }
                ...
                const allDocsProcessed =
                    extractedSgpas.length === sgpaDocs.length;
                ...
                if (
                    ...
                    const maxSemester = Math.max(
                        ...extractedSgpas.map((s) => s.semester)
                    );
                    const sum = extractedSgpas.reduce(
                        (acc, curr) => acc + curr.sgpa,
                        0
                    );

                    // If a student only uploads Sem 1 and 4, the divisor is 4, effectively counting gaps as 0.
                    const averageCgpa = parseFloat(
                        (sum / maxSemester).toFixed(2)
                    );
                    ...
                    await saveVerifiedAcademicData(userId, {
                        ...
                        verificationReason:
                            'Automated document extraction successful.',
                    });

                    console.log(
                        `[VERIFIED] Student ${userId} successfully verified. RollNo: ${detectedAstuRollNo}, CGPA: ${averageCgpa}, Backlogs: ${detectedBacklogSubjects.size}`
                    );
                } else {
                    const finalReason =
                    ...
                    console.log(
                        `[Verification Worker] Job FAILED for ${userId} -> Reason: ${finalReason}`
                    );
                }
    ...
    verificationWorker.on('failed', async (job, error) => {
        console.error(
            `[Verification Worker] Job ${job?.id} failed permanently:`,
            error.message
        );
        ...
        if (job?.data?.userId) {
            try {
                ...
                console.error(
                    `[Verification Worker] Failed to reset status in DB:`,
