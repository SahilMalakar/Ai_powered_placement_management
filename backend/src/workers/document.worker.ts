...
import { DOCUMENT_QUEUE_NAME, type DocumentJobPayload } from "../queues/document.queue.js";
...
export const initializeDocumentWorker = () => {
  const documentWorker = new Worker(
    ...
    async (job: Job<DocumentJobPayload>) => {
      ...
      console.log(`[Document Worker] Processing job ${job.id} for user ${userId} (${files.length} file(s))...`);

      for (const fileEntry of files) {
        const { fieldname, filePath, type, folder, semester } = fileEntry;
        const semesterUndefined = semester === null ? undefined : semester;
        let newPublicId: string | null = null;

        try {
          // 1. Conflict Check: Find existing doc for Cloudinary cleanup
          const existing = await findDocumentBySemester(userId, type, semesterUndefined);
          ...
          await upsertDocument(userId, type, cloudRes.secure_url, cloudRes.public_id, semesterUndefined);

          // 4. DB confirmed — delete old Cloudinary file
          if (oldPublicId) {
            deleteFromCloudinary(oldPublicId).catch((err) =>
              console.error(`[Document Worker] Cloudinary cleanup failed for ${oldPublicId}:`, err.message)
            );
          }

          console.log(`[Document Worker] ✅ ${fieldname} processed for user ${userId}`);
        } catch (err: any) {
          // Compensate: delete newly uploaded Cloudinary file if DB write fails
          if (newPublicId) {
            deleteFromCloudinary(newPublicId).catch((e) =>
              console.error(`[Document Worker] Orphan cleanup failed for ${newPublicId}:`, e.message)
            );
          }
          console.error(`[Document Worker] ❌ Failed to process ${fieldname}:`, err.message);
  ...
  documentWorker.on("failed", (job, error) => {
    console.error(`❌ Document job ${job?.id} failed for user ${job?.data.userId}:`, error.message);
  });

  documentWorker.on("completed", (job) => {
    console.log(`✅ Document job ${job.id} completed for user ${job.data.userId}`);
  });

  console.log(`[Document Worker] Initialized worker for queue: ${DOCUMENT_QUEUE_NAME}`);
};
