...
const STEPS = ["core", "social", "experience", "projects", "skills", "additional"] as const
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as Step)} className="w-full">
        ...
        <div className="bg-card shadow-card border border-border rounded-2xl p-6 md:p-8 min-h-[400px]">
          <TabsContent value="core" className="mt-0 focus-visible:outline-none">
            <CoreInfoTab onNext={handleNext} onSave={handleSave} initialData={formData} isSaving={isSaving} />
          </TabsContent>
          <TabsContent value="social" className="mt-0 focus-visible:outline-none">
            <SocialLinksTab onNext={() => handleNext()} onPrev={handlePrev} />
          </TabsContent>
          <TabsContent value="experience" className="mt-0 focus-visible:outline-none">
            <ExperienceTab onNext={() => handleNext()} onPrev={handlePrev} />
          </TabsContent>
          <TabsContent value="projects" className="mt-0 focus-visible:outline-none">
            <ProjectsTab onNext={() => handleNext()} onPrev={handlePrev} />
          </TabsContent>
          <TabsContent value="skills" className="mt-0 focus-visible:outline-none">
            <SkillsTab onNext={() => handleNext()} onPrev={handlePrev} isSaving={isSaving} />
          </TabsContent>
          <TabsContent value="additional" className="mt-0 focus-visible:outline-none">
            <AdditionalDetailsTab onPrev={handlePrev} onSave={handleSave} isSaving={isSaving} />
          </TabsContent>
        </div>
      </Tabs>
